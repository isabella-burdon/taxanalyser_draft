...
for full_ont_dir_path in $ont_directory_list; do
    ...
    mkdir -p "$cat_output_dir/$sub_runname"
    parallel --will-cite --colour --tag --eta -j 24 fix_corrupted_files_concat ::: "${file_basename_list[@]}"
done
...
parallel --will-cite --colour --tag --eta -j 24 final_concat ::: "${file_basename_list[@]}"
