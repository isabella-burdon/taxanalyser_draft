...
### GET DATA ###
assembly_directory = f'{data_root_path}/{run_name}/output/assembly'
pipestats_directory = f"{data_root_path}/{run_name}/output/pipeline_stats"
genome_db=f"reference_dbs/{db_name}/{db_name}_files"
taxonomy_db=f"reference_dbs/{db_name}/{db_name}.taxonomy.sqldb"

txt_filepath = f"{assembly_directory}/{folder}/flye_ouput/assembly_info.txt"
...
assembly_fasta_file = f"{assembly_directory}/{folder}/flye_ouput/assembly.fasta"
sourmash_output_dir = f'{assembly_directory}/{folder}'
sig_dir = f'{assembly_directory}/{folder}/sigs'
...
for sig_filename in sig_filenames:
    ...
    if os.path.exists(f'{output_file_basepath}_matches'):
        ...
        ident= f'BHI_ISMS_{species.replace(" ", "_")}_{run_name}_{output_file_basepath.split("/")[-3]}_{output_file_basepath.split("/")[-1]}'
